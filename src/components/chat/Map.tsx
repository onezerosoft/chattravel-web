...
const isRegion = (x: string): x is region =>
  Object.keys(REGION_MAP).includes(x);

const Map = () => {
  const [hoveredRegion, setHoveredRegion] = useState<region>("seoul-si");
  ...
  return (
    <Wrapper>
      <MapSVG handleMouseOver={handleMouseOver} />
...
const MapSVG = ({ handleMouseOver }: MapSvgProps) => (
  <svg
    width="229"
    height="405"
    viewBox="0 0 229 405"
